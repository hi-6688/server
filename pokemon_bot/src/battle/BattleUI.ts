...
function drawPokemonHUD(
  ...
  ctx.save();
  if (isTera) {
    ctx.strokeStyle = '#00F0FF'; // 太晶霓虹藍
    ctx.lineWidth = 4;
  } else if (isMega) {
    ctx.strokeStyle = '#FF64FF'; // Mega霓虹粉
    ctx.lineWidth = 4;
  } else {
    ctx.strokeStyle = '#000000'; // 經典黑框
    ctx.lineWidth = 3;
  }
  ...
  ctx.save();
  if (status) {
    drawStatusBadge(ctx, status, x + skew + 12, y + 48);
  }
