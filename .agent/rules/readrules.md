...
- **前端快取陷阱 (Frontend Cache Trap)**: 任何時候在 `main.py` 等後端分發 React 編譯後的 `index.html`，**必須強制加上 `Cache-Control: no-cache` 標頭**，嚴禁發生改了前端但使用者瀏覽器讀到舊快取的低級錯誤。
...
- **API / Backend**: 修改邏輯時，必須遵循 `docs/BACKEND_API.md`。新增路由時在 `api_routers/` 對應模組中加 handler，再到 `main.py` 註冊。
- **Global Deployment**: 查詢全域部署架構時，必須遵循 `docs/GLOBAL_DEPLOYMENT.md`。
