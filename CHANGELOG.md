...
- **統一 HUD 卡片為 80px 高度無邊框六邊形卡片**：重新調整我方與敵方 HUD 外框為統一 `80px` 高度、左右兩端向外突出呈對稱尖角（`<` 與 `>`）的精緻六邊形卡片。應使用者要求，**完全不要任何邊框 (No Border/Stroke)**，去除內外部所有描線與邊框，僅以底色塊與屬性切片圖示拼接呈無框極簡排版。
- **重構與整合屬性與異常狀態像素圖示切片**：
  - **升級為 PokéRogue 官方像素徽章底板**：廢除使用中文長方形 `types_zh-Hant.png` 徽章，改用 PokéRogue 官方帶有斜切角的 `pbinfo_player_type1/2.png` 與 `pbinfo_enemy_type1/2.png` 進行排版，達成 100% 官方視覺效果。
  - **垂直上下堆疊排版（貼在框外並留出縫隙）**：實作屬性徽章於六邊形外框的右側外面垂直堆疊（第一屬性在右上使用 `Type 1` 底板，第二屬性在右下使用 `Type 2` 底板）。並在主體與屬性底板之間**留出 4 像素的自然懸掛縫隙**，使主體與徽章無框分離，完美呈現官方對齊與間距。
  - **純深色（黑色塊）底板、後置特殊屬性並緊密貼合**：將 HUD 六邊形底板由上半深色/下半白色漸層，重構為一整塊統一的深色背景（黑色塊），且將主體黑色塊寬度縮小為 `w - 40` (350px) 以緊密貼合屬性徽章。同時，將特殊屬性（太晶化 `[太晶]` / 超級進化 `[MEGA]`）文字移至名稱與性別右側（**「名稱 性別 特殊屬性」順序**），將 3D 血條寬度微調至 `125px`、HP 數值位置對齊血條右端。
  - **異常狀態徽章切片**：將異常狀態（Statuses）徽章切片（源自 `statuses_zh-Hant.png`，放大至 `60x24`）精確排版於卡片底左下方。
