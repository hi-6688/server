...
- **統一 HUD 卡片為 80px 高度六邊形卡片**：重新調整我方與敵方 HUD 外框為統一 `80px` 高度、左右兩端向外突出呈對稱尖角（`<` 與 `>`）的精緻六邊形卡片（達成 100% 官方六邊形排版規格），並將我方 HUD 的繪製 Y 坐標微調至 `420`。
- **重構與整合屬性與異常狀態像素圖示切片**：
  ...
  - **垂直上下堆疊排版（貼在框外）**：實作屬性徽章於六邊形外框的右側外面垂直堆疊（第一屬性在右上使用 `Type 1` 底板，第二屬性在右下使用 `Type 2` 底板）。並根據六邊形斜邊的傾斜斜率與位置計算，使屬性徽章左側邊緣與六邊形右側黑框斜線無縫吻合，達成 100% 貼在框外的官方排版效果。
  - **去除白色漸層，改用純深色（黑色塊）底板並緊密貼合屬性**：將 HUD 六邊形底板由上半深色/下半白色漸層，重構為一整塊統一的深色背景（黑色塊），且將主體黑色塊寬度縮小為 `w - 40` (350px) 以緊密貼合屬性徽章。在主體與屬性徽章中間繪製一條縱向黑色分隔斜線，並將我方 HP 數值字體改為白字黑邊。同時，將 3D 血條寬度微調至 `125px`、HP 數值位置對齊血條右端以完全容納在主體內部。
