...
- **重構與整合屬性與異常狀態像素圖示切片**：
  ...
  - **垂直上下堆疊排版**：實作屬性徽章於右側傾斜插槽垂直堆疊（第一屬性在右上使用 `Type 1`，第二屬性在右下使用 `Type 2`）。並根據平行四邊形斜率，使下方的屬性徽章在 X 軸上精確向左位移偏置，使其完美貼合卡片傾斜邊緣。
  - **去除白色漸層，改用純深色（黑色塊）底板**：將 HUD 平行四邊形底板由上半深色/下半白色漸層，重構為一整塊統一的深色背景（黑色塊），並將我方 HP 數值字體改為白字黑邊以提高對比度與可讀性。
