...
- [x] **P1: 寶可夢對戰 HUD 排版重構與屬性切片繪製 (無等級/經驗值 Champion 版本完成)**
    ...
    - **統一卡片高度為 80px 的無邊框六邊形卡片**：重構 `drawPokemonHUD()`，我方與敵方卡片統一為 `80px` 高度、左右兩端對稱尖角的六邊形卡片，且**完全不要任何邊框**。
    - **整合屬性與底板圖示**：廢除 `types_zh-Hant.png` 徽章，升級為 PokéRogue 官方帶斜切角的 `pbinfo_player_type1/2.png` 與 `pbinfo_enemy_type1/2.png` 像素徽章，並於六邊形主體右側懸掛且**留出 4px 分離小縫隙**進行垂直上下堆疊排版。調整特殊屬性文字至名稱與性別右側（**「名稱 性別 特殊屬性」排版**），縮小主體寬度為 `w - 40` (350px) 以貼合屬性徽章，並微調血條寬度為 `125px` 與 HP 數值坐標對齊血條右側。
