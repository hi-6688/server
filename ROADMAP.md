...
- [x] **P1: 寶可夢對戰 HUD 排版重構與屬性切片繪製 (無等級/經驗值 Champion 版本完成)**
    ...
    - **整合屬性與底板圖示**：廢除 `types_zh-Hant.png` 徽章，升級為 PokéRogue 官方帶斜切角的 `pbinfo_player_type1/2.png` 與 `pbinfo_enemy_type1/2.png` 像素徽章，緊貼並懸掛於六邊形黑框的右側外面進行右上與右下垂直上下堆疊排版。將卡片主體黑色塊縮小為 `w - 40` (350px) 以緊密貼合屬性，在主體與屬性間繪製一條黑色分隔斜線，並將 3D 血條寬度微調為 `125px` 與 HP 數值坐標對齊血條右側以完全容納在主體內部，達成 100% 官方視覺效果。
