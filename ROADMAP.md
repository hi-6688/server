...
- [x] **P1: 寶可夢對戰 HUD 排版重構與屬性切片繪製 (無等級/經驗值 Champion 版本完成)**
    ...
    - **重構我方屬性置左與完美垂直對齊**：我方屬性置左懸掛貼合左斜邊，敵方屬性置右懸掛貼合右斜邊。兩者均在主體與屬性底板間預留 4 像素自然縫隙。第一與第二屬性上下堆疊，並精確計算 X 坐標確保兩個徽章呈完美直線垂直對齊。
    - **資訊排版與異常狀態整合**：特殊屬性（太晶化 `[太晶]` / 超級進化 `[MEGA]`）移至名稱與性別右側（**「名稱 性別 特殊屬性」順序**）。3D 血條寬度調整為 `125px`，HP 數值 `hp/maxHp` 收納在右下角 `y + 43` 內部。異常狀態徽章切片（60x24）精確排版於底左方（`y + 23`）。
    - **修復測試程式與編譯衝突**：在 `test_pokerogue_ui.ts` 中同步實作 50px 貼合與我方左置邏輯，並移除 [BattleUI.ts](file:///home/hi6688/servers/pokemon_bot/src/battle/BattleUI.ts) 中重疊的舊函數以清除語法地雷。
