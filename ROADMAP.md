...
- [x] **P1: 寶可夢對戰 HUD 排版重構與屬性切片繪製 (無等級/經驗值 Champion 版本完成)**
    ...
    - **整合屬性與底板圖示**：廢除 `types_zh-Hant.png` 徽章，升級為 PokéRogue 官方帶斜切角的 `pbinfo_player_type1/2.png` 與 `pbinfo_enemy_type1/2.png` 像素徽章，緊貼並懸掛於六邊形黑框的右側外面進行右上與右下垂直上下堆疊排版，左側斜線與六邊形黑框無縫貼合；並將 HUD 六邊形底板由原本下半部白色漸層重構為純深色背景（黑色塊），且將 HP 數值改為白字黑邊以求 100% 官方視覺效果。
