...
- [x] **P1: 寶可夢對戰 HUD 排裝重構與屬性切片繪製 (無等級/經驗值 Champion 版本完成)**
    - **移除等級與經驗值**：由於 Champion 版本不需要等級與經驗值，移除 `Lv.` 文字與我方的底部 `EXP` 經驗條。
    - **統一卡片高度為 50px 的無邊框六邊形卡片**：重構 `drawPokemonHUD()`，我方與敵方卡片統一為 `50px` 高度以完全貼合雙屬性徽章（高度 24px * 2 = 48px），完全移除任何邊框，採用純深色極簡六邊形設計。
    - **我方屬性置左與屬性無縫貼合**：我方屬性置左貼合左斜邊，敵方屬性置右貼合右斜邊。取消 4px 空隙，實作屬性徽章與卡片斜邊 100% 無縫貼合，呈完美積木積木拼接。第一與第二屬性上下堆疊，並精確計算 X 坐標確保兩個徽章呈完美直線垂直對齊。
    - **引入特殊狀態與進化 UI 素材**：載入官方 `icon_tera.png` 像素寶石（16x20 像素）與 `icon_mega.png` 螺旋標誌（16x16 像素）來繪製在寶可夢名字與性別右側，完全取代原本的 `[太晶]` 與 `[MEGA]` 純文字前綴。
    - **排版微調與測試程式同步**：3D 血條寬度調整為 `125px`，HP 數值 `hp/maxHp` 收納在右下角 `y + 43` 內部。異常狀態徽章切片（60x24）精確排版於底左方（`y + 23`）。在 `test_pokerogue_ui.ts` 中同步實作 50px 貼合、無縫隙計算與 UI 素材繪製，移除 [BattleUI.ts](file:///home/hi6688/servers/pokemon_bot/src/battle/BattleUI.ts) 中重疊的舊函數以清除語法地雷。
