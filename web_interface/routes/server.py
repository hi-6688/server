...
def handle_stop_post(handler, params, instance):
    """POST /stop — 透過 VM2 關閉伺服器"""
    try:
        if proxy_helpers.is_vm2_running():
            proxy_helpers.proxy_to_agent("execute_command", screen_name=instance.screen_name, command="stop")
            time.sleep(3)
            proxy_helpers.proxy_to_agent("execute_command", screen_name=instance.screen_name, command='\x03')
        handler._set_headers()
