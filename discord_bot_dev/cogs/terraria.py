...
# Config loaded below with settings
import json
...
class Terraria(commands.Cog):
    ...
    async def check_idle_loop(self):
        if not self.server_process:
            # Auto-discovery: Check if server is running
    ...
    @app_commands.command(
        name=CMD_CONFIG.get('cmd', {}).get('name', 'cmd'),
        description=CMD_CONFIG.get('cmd', {}).get('description', 'Send command')
    )
    @app_commands.describe(command_text="要執行的指令內容")
    @app_commands.rename(command_text="指令內容") 
    async def slash_cmd(self, interaction: discord.Interaction, command_text: str):
        if not await self.verify_permission(interaction, 'cmd'):
             return

        if self.server_process:
            await self.send_command(command_text)
            await interaction.response.send_message(f"已發送指令: `{command_text}`", ephemeral=True) 
        else:
    ...
    async def slash_status(self, interaction: discord.Interaction):
        ...
        if self.server_process:
            ...
            await interaction.followup.send(f"🔴 **離線** | 使用 `/泰亂啟動` 啟動伺服器")

    @app_commands.command(
        name=CMD_CONFIG.get('start', {}).get('name', 'start'),
        description=CMD_CONFIG.get('start', {}).get('description', 'Start server')
    )
    async def slash_start(self, interaction: discord.Interaction):
        if not await self.verify_permission(interaction, 'start'):
             return
        
        print(f"[DEBUG] slash_start triggered by user {interaction.user} in channel {interaction.channel_id}")

        if self.server_process:
            await interaction.response.send_message("伺服器已經在運作了！", ephemeral=True)
        else:
            await interaction.response.send_message(f"🚀 {BOT_NAME} 正在啟動伺服器... 請稍後片刻。")
    ...
    @app_commands.command(
        name=CMD_CONFIG.get('stop', {}).get('name', 'stop'),
        description=CMD_CONFIG.get('stop', {}).get('description', 'Stop server')
    )
    async def slash_stop(self, interaction: discord.Interaction):
        if not await self.verify_permission(interaction, 'stop'):
             return

        if self.server_process:
            await interaction.response.send_message("正在儲存並關閉伺服器...")
            await self.send_command("exit")
        else:
            await interaction.response.send_message("伺服器並未開啟。", ephemeral=True)
    ...
    async def run_terraria_server(self):
        ...
        if stdout:
            ...
            await self.send_log("🔍 檢測到伺服器已在後台運行，已自動恢復連線！") # Friendly message
            # Re-attach logic
            self.server_process = "SCREEN_SESSION" 
            if not self.server_task or self.server_task.done():
                self.server_task = self.bot.loop.create_task(self.read_output())
        ...
        await self.send_log(content=f"🟢 {BOT_NAME} 正在啟動伺服器 (Screen mode)...", view=HistoryView()) # Always send log
